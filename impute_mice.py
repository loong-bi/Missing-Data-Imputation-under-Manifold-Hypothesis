...
for index in range(len(xs_tests)):
    ...
    for iteration in range(max_mice_iters):
        ...
        for d in range(D):
            ...
            regressor = LinearRegression()
            #regressor = RandomForestRegressor(n_estimators=10, n_jobs=-1, random_state=321)
            # nonlinear mice is also very slow
    ...
    # # The previous evaluation protocol introduces transductive information leakage, as imputation is performed on the pooled dataset containing both training and test samples. In this work, we instead follow a standard inductive machine learning pipeline, where the imputation model is fitted using training data only and applied to unseen test data, ensuring a fair assessment of generalization performance.
